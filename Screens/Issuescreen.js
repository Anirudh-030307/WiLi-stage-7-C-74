...
export default class Issuescreen extends React.Component {
  ...
  CheckBookAvailability = async () => {
    const bookref = await db.collection('Books').where('BookName', '==', this.state.BookID).get()
    var TransactionType = ''
  ...
  CheckStudentEligibilityForBookIssue = async () => {
    const studentref = await db.collection('Students').where('StudentName', '==', this.state.StudentID).get()
